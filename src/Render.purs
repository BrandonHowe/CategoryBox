...
updateStateCache world geom action = case action of
  ...
  UpdateCreateCategory name -> Tuple newWorld newState
    where
      ...
      newState = geom { geometryCaches = snoc geom.geometryCaches $ unsafePerformEffect emptyGeometryCache
                      ...
                      , mainGeometryCache = uncurry (createForeignObject geom.mainGeometryCache) (bimap (mul 100) (mul 100) $ invertCantorPairing $ length $ (_.objects) $ getFunctorCategory world) ("id " <> name)
                      }
  ...
  where
    ...
    updateWorldCategory cat w =  world { categories = fromMaybe world.categories $ updateAt (geom.currentCategory - 1) cat w.categories }
...
canvasComponent world st = do
  ...
  fromMaybe (canvasComponent world st) $ newState <#> \passedState -> case passedState of
    (NewState state) -> canvasComponent (fst state) (snd state)
    (RaiseComponent component) -> canvasComponent world st <|> component >>= \state -> canvasComponent (fst state) (snd state)
